...
module.exports = function (enrollObj, g_options, fcw, logger) {
	...
	//check if chaincode exists 检查链码是否存在
	...
	//check chaincode version 检查链码版本
	...
	marbles_chaincode.create_a_marble = function (options, cb) {
		...
		logger.info('————————Tenney————————创建大理石————————Tenney————————');
	...
	// Owners -------------------------------------------------------------------------------
	// 创建用户
	marbles_chaincode.put_user = function (options, cb) {
		logger.info('____Tenney____创建用户____Tenney____');
		var opts = {
			peer_urls: g_options.peer_urls,
			peer_tls_opts: g_options.peer_tls_opts,
			channel_id: g_options.channel_id,
			...
			cc_args: [
				...
				options.args.username,
				options.args.company,
				options.args.password
			],
		};
		fcw.invoke_chaincode(enrollObj, opts, function (err, resp) {
			if (cb) {
	...
	marbles_chaincode.register_owner = function (options, cb) {
		...
		console.log('___Tenney___',options.args.marble_owner);
		var opts = {
			peer_urls: g_options.peer_urls,
			peer_tls_opts: g_options.peer_tls_opts,
			channel_id: g_options.channel_id,
	...
	//owner_identity //权限分配
	marbles_chaincode.identity = function (options, cb) {
		logger.info('Creating a marble owner...');
		console.log('___Tenney___权限分配');
		var opts = {
			peer_urls: g_options.peer_urls,
			peer_tls_opts: g_options.peer_tls_opts,
			channel_id: g_options.channel_id,
			...
			endorsed_hook: options.endorsed_hook,
			ordered_hook: options.ordered_hook,
			cc_function: 'owner_identity',
			cc_args: [
				options.args.wner_id,
				options.args.compan
			],
		};
		fcw.invoke_chaincode(enrollObj, opts, function (err, resp) {
	...
	//审批
	marbles_chaincode.approval = function (options, cb) {
		logger.info('Creating a marble owner...');
		console.log('___Tenney___审批');
		var opts = {
			peer_urls: g_options.peer_urls,
			peer_tls_opts: g_options.peer_tls_opts,
			channel_id: g_options.channel_id,
	...
	//物品上架
	marbles_chaincode.on_shelf = function (options, cb) {
		logger.info('Creating a marble owner...');
		console.log('___Tenney___物品上架');
		var opts = {
			peer_urls: g_options.peer_urls,
			peer_tls_opts: g_options.peer_tls_opts,
			channel_id: g_options.channel_id,
			...
			endorsed_hook: options.endorsed_hook,
			ordered_hook: options.ordered_hook,
			cc_function: 'on_shelf',
			cc_args: [
				options.args.marble_id,
				options.args.want_color,
				'a' + leftPad(Date.now() + randStr(5), 19)
			],
		};
		fcw.invoke_chaincode(enrollObj, opts, function (err, resp) {
			if (cb) {
	...
	//物品交换
	marbles_chaincode.exchange = function (options, cb) {
		logger.info('Creating a marble owner...');
		console.log('___Tenney___物品交换');
		var opts = {
			peer_urls: g_options.peer_urls,
			peer_tls_opts: g_options.peer_tls_opts,
			channel_id: g_options.channel_id,
			...
			endorsed_hook: options.endorsed_hook,
			ordered_hook: options.ordered_hook,
			cc_function: 'exchange',
			cc_args: [
				options.args.auction_id,
				options.args.ex_marble_id
			],
		};
		fcw.invoke_chaincode(enrollObj, opts, function (err, resp) {
			if (cb) {
