...
module.exports = function (config_filename, logger) {
	...
	if (!config_filename) {
		config_filename = 'marbles_tls.json';										// 默认配置文件名
	}
	cp.config_path = path.join(__dirname, '../../config/' + config_filename);
	cp.config = require(cp.config_path);											// 加载配置文件
	logger.info('Loaded config file', cp.config_path);								// 配置文件路径

	// --------------------------------------------------------------------------------
	//检测连接配置文件数据是否处于环境变量而不是文件
	// --------------------------------------------------------------------------------
	let use_env = detect_env.getConnectionProfileFromEnv();
	if (use_env) {																	// 使用Env或不使用Env
		cp.using_env = true;
		...
		cp.creds = require(cp.cp_path);												// 加载凭证文件
	...
	cp.makeEnrollmentOptions = function (userIndex) {
		if (userIndex === undefined || userIndex == null) {
			...
			const user_obj = cp.getEnrollObj(first_ca, userIndex);		//可能有多个用户
			return {
				channel_id: channel,
	...
	cp.write = function (obj) {
		...
		if (obj.chaincodeId) {
			...
			if (obj.chaincodeVersion) {						// 改变ID和版本
		...
		try {
			fs.writeFileSync(cp.cp_path, JSON.stringify(creds_file, null, 4), 'utf8');	//保存到文件
		} catch (e) {
			logger.error('could not write file', cp.cp_path, e);
		}
		cp.creds = creds_file;															//替换旧拷贝
	};
