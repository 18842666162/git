...
type Marble struct {
	...
	Status     int           `json:"status"` // 1 无限制状态 2 审批状态
	Value      float64       `json:"value"`
	Want       string        `json:"want"` //资产价值
...
type Owner struct {
	...
	Identity   int     `json:"identity"` //身份 1为普通用户2为审批员
	Company    string  `json:"company"`
	Credit     float64 `json:"credit"` //用户积分
	Status     bool    `json:"status"` //disabled owners will not be visible to the application
}
...
type Auction struct {
	...
	Marble     MarbleRelation `json:"marble"`
}

type MarbleRelation struct {
	Id    string  `json:"id"`
	Color string  `json:"color"`
	Value float64 `json:"value"`
}
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "login" { // deng lu  ****yong hu id****  ****yong hu mi ma*****
		return login(stub, args)
	} else if function == "test_set_owner" { // zi can zhuan yi
		return test_set_owner(stub, args)
	} else if function == "owner_identity" { // 设置审批员
		return owner_identity(stub, args)
	} else if function == "approval_marble" { // 审批大理石状态
		return approval_marble(stub, args)
	} else if function == "on_shelf" { // 商品上架
		return on_shelf(stub, args)
	} else if function == "exchange" { // 商品交换
		return exchange(stub, args)
	} else if function == "lower_shelf" { // 商品下架
		return lower_shelf(stub, args)
	}
	// error out
