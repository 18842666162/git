...
func get_user(stub shim.ChaincodeStubInterface, id string) (User, error) {
	...
	if len(user.Username) == 0 { //test if owner is actually here or just nil
		return user, errors.New("User does not exist - " + id + ", '" + user.Username + "' '" + user.Password + "'")
	}
...
func Two(v float64) (float64, error) {
	return strconv.ParseFloat(fmt.Sprintf("%.2f", v), 2)
}
