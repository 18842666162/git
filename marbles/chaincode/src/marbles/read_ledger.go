...
func read_everything(stub shim.ChaincodeStubInterface) pb.Response {
	...
	for ownersIterator.HasNext() {
		...
		if owner.Status { //only return enabled owners
			everything.Owners = append(everything.Owners, owner) //add this marble to the list
		}
	}
	fmt.Println("owner array - ", everything.Owners)

	auctionIterator, err := stub.GetStateByRange("a0", "a9999999999999999999")
	if err != nil {
		return shim.Error(err.Error())
	}
	defer auctionIterator.Close()

	for auctionIterator.HasNext() {
		aKeyValue, err := auctionIterator.Next()
		if err != nil {
			return shim.Error(err.Error())
		}
		queryKeyAsStr := aKeyValue.Key
		queryValAsBytes := aKeyValue.Value
		fmt.Println("on auction id - ", queryKeyAsStr)
		var auction Auction
		json.Unmarshal(queryValAsBytes, &auction) //un stringify it aka JSON.parse()

		// if auction.Status { //only return enabled owners
		everything.Auction = append(everything.Auction, auction) //add this marble to the list
		// }
	}
	fmt.Println("auction array - ", everything.Auction)
...
func login(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	uid = args[0]
	//password = args[1]
	valAsbytes, err := stub.GetState(uid) //get the var from ledger
	if err != nil {
	...
	// 	jsonResp = "{\"Error\":\"password incorrect \"}"
	// 	return shim.Error(jsonResp)
	// }

	jsonResp = "{\"success\":\" " + user.Password + " \"}"

	fmt.Println("- deng lu cheng gong")
	return shim.Success(valAsbytes) //send it onward
}
